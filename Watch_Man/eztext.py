...
class Button:
    def __init__(self, **options):
        self.options = Config(options, ['caption', '\'\''], ['pos', '(0,0)'], ['font', 'pygame.font.Font(None, 24)'],
                              ['color', '(255,255,255)'], ['boxColor', '(253,131,32)'], ['length', '16'], ['width', '16'] )
        self.pos = self.options.pos
        self.caption = self.options.caption
        self.pause = 0
        self.font = self.options.font
        self.color = self.options.color
        self.boxColor = self.options.boxColor
        self.outlineColor = WHITE
        self.box = pygame.Rect(self.pos[0], self.pos[1], self.options.length, self.options.width)

    def draw(self, surface):
        pygame.draw.rect(surface,self.boxColor, self.box, 0)
        pygame.draw.rect(surface,self.outlineColor, self.box, 2)
        text = self.font.render(self.caption, 1, self.color)
        surface.blit(text, (self.pos[0] + 4, self.pos[1] + 6 ))

    def onClicked(self, events):
        for event in events:
            if event.type == MOUSEBUTTONUP:
                mouspos = pygame.mouse.get_pos()
                if self.box.collidepoint(mouspos):
                    return True
                else: return False
...
class Input:
    ...
    def __init__(self, **options):
        ...
        self.options = Config(options, ['x', '0'], ['y', '0'], ['font', 'pygame.font.Font(None, 24)'],
                              ['color', '(0,0,0)'], ['restricted', '\'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!"#$%&\\\'()*+,-./:;<=>?@[\]^_`{|}~\''],
                              ['maxlength', '-1'], ['prompt', '\'\''], ['length', '0'], ['width', '0'])
        ...
        self.box = pygame.Rect(self.x,self.y,self.options.length,self.options.width)
        self.boxColor = BLUE
        self.pause = 0
    ...
    def set_pos(self, x, y):
        ...
        self.x = x
        self.y = y
        self.box = pygame.Rect(x,y,self.options.length,self.options.width)
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface,self.boxColor, self.box, 2)
        text = self.font.render(self.value, 1, self.color)
        caption = self.font.render(self.prompt, 1, PINK)
        surface.blit(caption, (self.x, self.y-16))
        surface.blit(text, (self.x + 4, self. y + 6 ))
        ...
        for event in events:
            if event.type == MOUSEBUTTONUP:
                ...
                if self.box.collidepoint(mouspos):
                    ...
                    self.boxColor = BLUE
        if(self.isSelected):
            pressed = pygame.key.get_pressed() #allow ability to hold down key to delete
